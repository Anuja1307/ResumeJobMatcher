...
try {
    ...
    console.log(
        "===== JOB RAG INDEXING COMPLETE ====="
    );

} catch (ragErr) {

    console.error(
        "Job RAG indexing failed:",
        ragErr.message
    );
...
exports.updateJob = async (req, res) => {
    try {
        ...
        if (location !== undefined && location !== job.location) {
        ...
        if (experience !== undefined && experience !== job.experience) {
        ...
        // ======================================================
        // REPROCESS JOB AI DATA
        // ======================================================

        if (contentChanged || !job.embedding || job.embedding.length === 0) {

            // Re-extract skills and keywords
            try {

                const aiResult = await extractJobInformation({
                    title: job.title,
                    description: job.description,
                    experience: job.experience
                });

                job.requiredSkills = aiResult.requiredSkills;
                job.keywords = aiResult.keywords;

            } catch (aiErr) {

                console.error(
                    "Job information extraction failed:",
                    aiErr.message
                );
            }
...
Keywords: ${(job.keywords || []).join(", ")}
                ...
                job.embedding =
                    await generateEmbedding(jobText);

            } catch (embedErr) {

                console.error(
                    "Embedding generation failed during job update:",
                    embedErr.message
                );
            }
        }
